    ...
    for (let i = 0; i < environmentArray.length; i++) {
        ...
        for (let j = 0; j < environmentArray.length; j++) {
            ...
            if (actualLocalAddress !== undefined) {
                ...
                console.log(
                    `${environmentArray[j]}'s currentSetRemoteAddress for ${environmentArray[i]}: ${currentlySetTrustedRemote} ${JSON.stringify(actualSetTrustedRemote) === JSON.stringify(currentlySetTrustedRemote) ? "✅ " : "❌ "
                    }`
                )
